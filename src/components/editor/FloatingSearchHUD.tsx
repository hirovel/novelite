  ...
  const handleReplaceAll = () => {
    if (view && searchText) {
      ...
      eventBus.emit('show-toast', {
        message: isEn ? `Replaced ${totalReplaced} occurrences` : `全篇替换完成：共替换 ${totalReplaced} 处`,
        type: 'success',
      });
      setMatchCount({ current: 0, total: 0 });
    }
    ...
    >
      ...
      <div className="flex items-center gap-1.5 p-2">
        <button
          ...
          title={mode === 'replace' ? (isEn ? 'Collapse Replace Bar' : '折叠替换栏') : (isEn ? 'Expand Replace Bar (Ctrl+H)' : '展开替换栏 (Ctrl+H)')}
        >
        ...
        <div className="relative flex-1">
          <input
            ...
            placeholder={isEn ? 'Find text...' : '查找文本...'}
        ...
        {searchText && (
          <span className="text-[10px] font-mono opacity-60 shrink-0 px-1.5">
            {matchCount.total > 0 ? `${matchCount.current}/${matchCount.total}` : (isEn ? 'No results' : '无结果')}
          </span>
        )}
        ...
        <div className="flex items-center gap-0.5">
          <button
            ...
            title={isEn ? 'Previous Match (Shift+Enter)' : '上一个 (Shift+Enter)'}
          >
          ...
          <button
            onClick={handleNext}
            className="p-1 rounded-lg hover:bg-white/10 opacity-70 hover:opacity-100 transition-all cursor-pointer"
            title={isEn ? 'Next Match (Enter)' : '下一个 (Enter)'}
          >
        ...
        <button
          ...
          title={isEn ? 'Match Case' : '区分大小写 (Match Case)'}
        >
        ...
        <button
          ...
          title={isEn ? 'Match Whole Word' : '全词匹配 (Whole Word)'}
        >
        ...
        <button
          ...
          className="p-1 rounded-lg opacity-50 hover:opacity-100 hover:bg-white/10 transition-all cursor-pointer"
      ...
      {mode === 'replace' && (
        <div className="flex items-center gap-2 p-2.5 pt-0 border-t border-white/5 animate-in fade-in duration-150">
          ...
          <input
            ...
            placeholder={isEn ? 'Replace with...' : '替换为...'}
            className="flex-1 rounded-xl px-2.5 py-1.5 text-xs outline-none border font-sans"
            style={{ backgroundColor: theme.colors.bg, borderColor: `${theme.colors.border}60`, color: theme.colors.text }}
          />
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-lg text-[11px] font-medium bg-white/5 hover:bg-white/10 border border-white/10 transition-all disabled:opacity-30 cursor-pointer"
            title={isEn ? 'Replace Current Match' : '替换当前匹配'}
          >
            {isEn ? 'Replace' : '替换'}
          </button>
          ...
          <button
            ...
            title={isEn ? 'Replace All Occurrences' : '全篇批量替换'}
          >
            <CheckCheck className="h-3 w-3" />
            <span>{isEn ? 'All' : '全换'}</span>
          </button>
        </div>
      )}
    </div>
